...
def time_func(f, args, kwargs):
    ...
    print(
        f'>>> function {f.__name__} from {f.__module__} took {t1-t0:.6f} seconds')
    return res
...
def compare_values(g, edge_dict):
    ...
    rust_res4 = rustpyppr.multiple_forward_push_vec(
        edge_dict, [0], 0.85, 1e-6)[0]
    rust_res5 = rustpyppr.multiple_forward_push_vec_lazy(
        edge_dict, [0], 0.85, 1e-6)[0]
    print_compare(nx_res, rust_res1, rust_res2,
                  rust_res3, rust_res4, rust_res5)
...
def test_my_random():
    random.seed(0)
    edge_dict = create_random_graph(20_000)
